...
- **JP (日本版)**: 全39問（資産、健康、キャリア、時間・環境）
- **US (米国版)**: 全19問（401k/IRA, HSA, Credit Score, Debt, Impulse Buying, etc.）※v2.4.0で拡充

（以下、JP版の質問リスト詳細は省略。US版はコード参照）
...
| 2026-02-10 | 2.4.0 | 多言語対応（i18n）実装（日/米）。米国版質問セット(US_QUESTIONS)の大幅拡充（全19問追加）。NISA/iDeCo損失計算ロジックの修正。AccordionCardのプロパティ修正。 |

## 13. 国際化 (i18n) 設計

### 13.1 対応言語・リージョン
- **日本語 (ja-JP)**: デフォルト。地域 `JP` に対応。
- **英語 (en-US)**: 地域 `US` に対応。
- **ロケール検知**: ブラウザの言語設定 (`navigator.language`) または IPアドレス（Vercel Headers）から自動判定し、初回アクセス時に設定。`localStorage` に設定を保存。

### 13.2 翻訳管理
- `lib/i18n.ts`: 辞書オブジェクトによるテキスト管理。
- UIコンポーネントは `locale` プロパティを受け取り、動的にテキストを切り替え。
- 通貨表示: `Intl.NumberFormat` を使用し、円 (`JPY`) とドル (`USD`) を適切にフォーマット。

### 13.3 リージョン別質問セット
`lib/questions.ts` にて `REGION_QUESTIONS` を定義。
- `JP`: 日本の制度（NISA, iDeCo, ふるさと納税）に基づく質問。
- `US`: 米国の制度（401k, Roth IRA, HSA, Credit Score）に基づく質問。
- 共通のロジック (`AuditEngine`) を使用しつつ、通貨単位や係数、推奨アクションをリージョンごとに最適化。
